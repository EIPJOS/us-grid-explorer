...
const pages = {
  about: {
    ...
    <section><h2>Current development status</h2><p>The project is actively developed. National map layers and state profiles are available now; local reports and facility profiles are planned. The optional Grid Guide remains dependent on separately configured API access and is not required to use the site.</p></section>`
  },
  methodology: {
