...
import { guideRoutes, rankingRoutes, renderAnalyticsScript, renderSiteFooter, renderSiteHeader, siteUrl, trustRoutes } from "./site-shell.mjs";
...
function renderSitemap() {
  ...
  const urls = [siteUrl, `${siteUrl}/states/`, `${siteUrl}/guides/`, ...guideRoutes.map((route) => `${siteUrl}/guides/${route}/`), `${siteUrl}/rankings/`, ...rankingRoutes.map((route) => `${siteUrl}/rankings/${route}/`), ...Object.values(profiles).map((profile) => `${siteUrl}/states/${profile.slug}/`), ...plantUrls, ...trustRoutes.map((route) => `${siteUrl}/${route}/`)];
